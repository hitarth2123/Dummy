...
export default function Navbar({ onMenuClick, collapsed, onToggleCollapse }) {
  ...
  return <header className="sticky top-0 z-30 h-16 bg-surface/85 shadow-[0_1px_8px_rgba(0,0,0,0.18)] backdrop-blur-xl"><div className="flex h-16 items-center gap-4 px-4 sm:px-6"><div className="flex shrink-0 items-center gap-3"><button type="button" aria-label="Open navigation" className="rounded-lg p-2 text-on-surface-variant hover:bg-surface-container lg:hidden" onClick={onMenuClick}><Menu size={21} /></button><button type="button" aria-label="Toggle sidebar" className="hidden rounded-xl border border-surface-variant/30 p-2 text-on-surface-variant hover:bg-surface-container lg:flex" onClick={onToggleCollapse}>{collapsed ? <PanelLeftOpen size={19} /> : <PanelLeftClose size={19} />}</button><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-primary-container text-on-primary-container">✦</span><div><p className="font-bold leading-tight text-primary">AI Buddy</p><p className="hidden text-[10px] text-on-surface-variant sm:block">{roleLabels[role]}</p></div></div><span className="hidden rounded-full bg-surface-container-high px-2 py-0.5 text-[10px] text-secondary xl:inline-block">{user?.department || 'Institutional'}</span></div><div className="mx-1 hidden min-w-0 flex-1 items-center rounded-xl bg-surface-container-low px-4 py-1.5 md:flex"><Search size={16} className="mr-2 shrink-0 text-outline" /><input className="w-full bg-transparent text-sm text-on-surface outline-none placeholder:text-outline" placeholder="Search curriculum, topics, exams..." /></div><div className="flex shrink-0 items-center gap-3">
    {notificationButton}
