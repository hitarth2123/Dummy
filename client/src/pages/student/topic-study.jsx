  ...
  return (
    <section id="topic-mcq-checkpoint" className="space-y-6">
      ...
      <header className="rounded-3xl bg-ink p-6 text-white shadow-panel sm:p-8">
        ...
        <div className="mt-4 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm text-outline">Topic {topic.order || ''}</p><h1 className="mt-1 text-3xl font-bold tracking-tight">{topic.topic}</h1><p className="mt-2 max-w-2xl text-sm text-outline">{topic.description || `Build your understanding of ${topic.topic} before testing your knowledge.`}</p></div><div className="flex flex-wrap gap-2"><Link to={practiceLink} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-secondary px-4 py-3 text-sm font-bold text-on-surface transition hover:bg-secondary"><HelpCircle size={17} /> Practice MCQs</Link><button type="button" onClick={handleDone} disabled={saving || topic.status === 'completed'} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border border-white/20 bg-surface-container-low/10 px-4 py-3 text-sm font-bold text-white transition hover:bg-surface-container-low/20 disabled:cursor-not-allowed disabled:opacity-70"><CheckCircle2 size={17} /> {saving ? 'Saving...' : topic.status === 'completed' ? 'Completed' : 'Done'}</button></div></div>
      </header>
